...
impl<T> TreeNode<T> for RefNode<T> where T: TreeNode<T> + TreeDataProvider<T> {
    ...
    fn children(&self) -> Vec<RefNode<T>> {
        let td = self.tree_data();
        let td = RefCell::borrow(&td);
        let tr = &td.tree_registry;

        if let Some(r) = tr {
            return td.children_id.iter()
                .map(|id| {
                    RefCell::borrow(r).get(id).cloned()
                })
                .filter(|o| { o.is_some() })
                .map(|o| { o.unwrap().clone() })
                .collect::<Vec<RefNode<T>>>();
        }
        vec![]
    }
...
impl<T> TreeData<T> where T: TreeNode<T> {
    ...
    fn children(&self) -> Vec<RefNode<T>> {
        if let Some(r) = &self.tree_registry {
            return self.children_id.iter()
                .map(|id| {
                    RefCell::borrow(r).get(id).cloned()
                })
                .filter(|o| { o.is_some() })
...
impl<T: TreeDataProvider<T>> TreeNode<T> for T {
    ...
    fn children(&self) -> Vec<RefNode<T>>{
        ...
        if let Some(r) = &td.tree_registry {
            return td.children_id.iter()
                .map(|id| {
                    RefCell::borrow(r).get(id).cloned()
                })
